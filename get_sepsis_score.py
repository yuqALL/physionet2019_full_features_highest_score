...
def kind_feature_filter(pos, dele=True):
    ...
    # auto_var_12h = 940
    # auto_var_24h = 974
    # auto_var_coef_1h = 1008
    # auto_var_coef_12h = 1042
    # auto_var_coef_24h = 1076

    skewness = 1110
    kurtosis = 1144
    extreme_measure = 1178
...
f_index=kind_feature_filter([5,7])
# 输入所有数据特征
def genFeature(data):
    global All_grad1, All_grad12, All_grad24, All_hess1,f_index
    exlen = 34
    # feature = data[:, :-1]
    feature = data
    h, w = feature.shape
    ...
    for j in range(w):
        for i in range(h):
            if np.isnan(feature[i, j]):
                feature[i, j] = searchNearValue(i, feature[:, j])
    ...
    # feature = cov
    skewness = skewness_res(res)
    kurtosis = kurtosis_res(res)
    extreme_measure = fMax - fMin
    stat_append = np.hstack((skewness, kurtosis, extreme_measure))
    # print(grad.shape)
    # print(hess.shape)
    # print(mu.shape)
    # print(stat.shape)
...
def ImputerMissingValue(feature, start=0, way='median'):
    ...
    for i in range(h):
        for j in range(w - start):
            if np.isnan(feature[i, j + start]) or np.isinf(feature[i, j + start]):
...
def skewness_res(res_f):
    global pre_power_3_skewness, pre_power_2_skewness
    res = np.copy(res_f)
    h, w = res.shape
    # sn = np.zeros(w)
    if h == 1:
        pre_power_3_skewness = np.zeros(w)
        pre_power_2_skewness = np.zeros(w)
    pre_power_3_skewness = np.vstack((pre_power_3_skewness, np.power(res[-1], 3)))
    pre_power_2_skewness = np.vstack((pre_power_2_skewness, np.power(res[-1], 2)))
    up = math.sqrt(h) * np.nansum(pre_power_3_skewness, axis=0)
    down = np.power(np.nansum(pre_power_2_skewness, axis=0), 1.5)
    for j in range(w):
        if down[j] < 0.00001:
            down[j] = 0.0001
    sn = up / down
    return np.reshape(sn, (1, w))
...
def kurtosis_res(res_f):
    global pre_power_4_kurtosis, pre_power_2_kurtosis
    res = np.copy(res_f)
    h, w = res.shape
    if h == 1:
        pre_power_4_kurtosis = np.zeros(w)
        pre_power_2_kurtosis = np.zeros(w)
    pre_power_4_kurtosis = np.vstack((pre_power_4_kurtosis, np.power(res[-1], 4)))
    pre_power_2_kurtosis = np.vstack((pre_power_2_kurtosis, np.power(res[-1], 2)))
    up = h * np.nansum(pre_power_4_kurtosis, axis=0)
    down = np.power(np.nansum(pre_power_2_kurtosis, axis=0), 2)
    for j in range(w):
        if down[j] < 0.00001:
            down[j] = 0.0001
    ks = up / down - 3
    return np.reshape(ks, (1, w))
...
def extremum_measure_sofvariation(res_f):
    ...
    max_res = np.nanmax(res, axis=0)
    min_res = np.nanmin(res, axis=0)
    es = max_res - min_res
    return np.reshape(es, (1, w))
